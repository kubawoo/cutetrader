...
/**
     ...
     */
inline bool argContain(QStringList args, QString argToFind, int &argIndex, int startIndex = 0)
{
...
{
    if (sl.access() != QMetaMethod::Private
        || sl.parameterTypes().size() != 0
        /*|| sl.returnType() != QMetaType::Void*/
...
inline TestCasesList selectTestCasesToRun(QStringList &arguments)
{
    ...
    if (argContain(arguments, "-case", argCaseIndex) && (argCaseIndex + 1) < arguments.size()) {
        ...
        while (argContain(arguments, "-case", argCaseIndex)
               && (argCaseIndex + 1) < arguments.size()) {
...
{
    int argIndex = 0;
    while (argContain(arguments, "-o", argIndex, argIndex) && (argIndex + 1) < arguments.size()) {
        ...
        // Add new suffix
        newFilename.append(QString("_%1").arg(caseIndex, 4, 10, QChar('0')));
        if (oldFilename.contains(".")) {
...
inline int run(int argc, char *argv[])
{
    ...
    qWarning() << qPrintable(QString("Started at %1").arg(start.toString()));
    ...
    if (ret > 0) {
        ...
        qCritical() << qPrintable(QString("%1 error in %2 case(s), over a total of %3 tests cases")
                                      .arg(ret)
                                      .arg(failedTestCase.size())
                                      .arg(casesToRun.size()));
    ...
    qWarning() << qPrintable(QString("Finished at %1").arg(finish.toString()));
    qWarning() << qPrintable(QString("Executed in %1 seconds (%2 ms)").arg(nbSecs).arg(nbMsecs));
...
#define QVERIFY_EXCEPTION_THROWN(func, exceptionClass) \
    { \
        ...
        if (!caught) { \
            QFAIL(qPrintable(QString("Nothing thrown where expected"))); \
        } \
    }
#endif
